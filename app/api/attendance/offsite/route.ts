...
export async function POST(req: NextRequest) {
  ...
  const day_local = body?.day_local ? String(body.day_local) : null; // YYYY-MM-DD (optional)
  ...
  // allow creating OFFSITE for a specific day (e.g. backfill "nákup materiálu")
  const dayOk = day_local && /^\d{4}-\d{2}-\d{2}$/.test(day_local);
  const dayFinal = dayOk ? day_local! : dayLocalCZNow();

  // server_time is mainly for ordering; keep it within the selected day if provided
