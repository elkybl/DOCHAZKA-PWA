...
export async function POST(req: NextRequest) {
  ...
  const allow_without_location = body?.allow_without_location === true;
  ...
  if (Number.isFinite(lat) && Number.isFinite(lng) && site.lat != null && site.lng != null) {
    distance_m = Math.round(haversineMeters({ lat: Number(lat), lng: Number(lng) }, { lat: Number(site.lat), lng: Number(site.lng) }));
    const radius_m = Number(site.radius_m || 0);
    if (!allow_without_location && radius_m > 0 && distance_m > radius_m) {
      return json({ error: `Jsi mimo radius stavby (${distance_m} m > ${radius_m} m).` }, { status: 403 });
    }
  } else if (!allow_without_location) {
  ...
  const { error } = await db.from("attendance_events").insert(payload);
